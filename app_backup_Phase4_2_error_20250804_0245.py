...
def update_user_stats(is_correct, question_id):
    ...
    if is_correct:
        ...
        session['learning_progress']['correct_answers'].add(question_id)
    else:
        session['learning_progress']['wrong_answers'].add(question_id)
...
def home():
    ...
    stats_data = {
        'ins_questions': 1379,
        'exam_questions': len(learning_progress['completed_questions']),
        'total_questions': 1379
    }
    
    # 진도 데이터 (세션 기반)
    progress_data = {
        'completed_questions': len(learning_progress['completed_questions']),
        'overall_progress_rate': round((len(learning_progress['completed_questions']) / 1379) * 100, 1),
